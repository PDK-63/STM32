...
//su dung 2 bang p10 32x16 tong la 2*32*16 = 1024 diem anh, tuong ung 128 byte
#define SO_DOT_TREN_HANG 32
#define SO_DOT_TREN_COT 16
#define SO_DOT_TREN_BANG SO_DOT_TREN_HANG*SO_DOT_TREN_COT

#define SO_BANG_SU_DUNG 2 // so bang led su dung tren 1 hang
#define SO_HANG_SU_DUNG 8 // so hang led su dung

#define SO_BUFFER_RAM (SO_BANG_SU_DUNG*SO_DOT_TREN_BANG)/8
#define SO_BYTE_TREN_HANG (SO_DOT_TREN_HANG*SO_BANG_SU_DUNG)/8

void led_init(void);
void quetledMT(void);
u8 write_string(u8 font,u8 *data, u8 x, u8 y);
extern TIM_HandleTypeDef htim1;
extern uint8_t flag_hetBH;

extern uint8_t buffer_quet1[SO_BUFFER_RAM];
extern uint8_t buffer_quet2[SO_BUFFER_RAM];
extern uint8_t buffer_quet3[SO_BUFFER_RAM];
extern uint8_t buffer_quet4[SO_BUFFER_RAM];
extern uint8_t buffer_quet5[SO_BUFFER_RAM];
extern uint8_t buffer_quet6[SO_BUFFER_RAM];
extern uint8_t buffer_quet7[SO_BUFFER_RAM];
extern uint8_t buffer_quet8[SO_BUFFER_RAM];
void setpixel(uint16_t x,uint16_t y,uint8_t data);
